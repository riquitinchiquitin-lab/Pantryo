...
export const Fido2AuthModal: React.FC<Fido2AuthModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && targetUser) {
      ...
      fido2Client
        ...
        .catch((err) => {
          setErrorMessage(err.message || 'Failed to load FIDO2 status');
        })
        .finally(() => {
          setLoading(false);
        });
    }
  }, [isOpen, targetUser, lang]);

  if (!isOpen || !targetUser) return null;

  // Handle FIDO2 Hardware or Sandbox Authentication
  const handleAuthenticate = async (simulated: boolean = false) => {
    try {
  ...
  const handleEnrollKey = async (simulated: boolean = false) => {
    try {
      ...
      if (result.success && result.verified) {
        setSuccessMessage(
          lang === 'FR'
            ? 'Clé FIDO2 enregistrée avec succès ! Votre compte est désormais conforme.'
            : 'FIDO2 Key enrolled successfully! Your account is now compliant.'
        );
  ...
  const handleDownloadCodes = () => {
    ...
    const content =
      ...
      `# Policy: All users must use FIDO2\n` +
    ...
    a.href = url;
    a.download = `pantryo-fido2-recovery-codes-${targetUser.name.toLowerCase()}.txt`;
    a.click();
  ...
  const isMandatoryEnrollment =
    ...
    fido2Status?.credentialsCount === 0;
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-fade-in">
      <div className="bg-[#FAF7EE] text-[#133E3B] w-full max-w-md rounded-3xl border border-[#E0D9C8] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="flex items-center gap-1.5">
                <h3 className="font-bold text-sm text-[#0D3B37]">
                  {isMandatoryEnrollment
                    ? lang === 'FR'
                      ? 'Enrôlement FIDO2 Obligatoire'
                      : 'Mandatory FIDO2 Enrollment'
                    : lang === 'FR'
                ...
                <span className="px-1.5 py-0.5 rounded-full text-[9px] font-black bg-teal-100 text-teal-800 border border-teal-200">
                  {lang === 'FR' ? 'TOUS UTILISATEURS' : 'ALL USERS'}
                </span>
              </div>
              <p className="text-[11px] text-[#527470]">
                {lang === 'FR'
                  ? `Profil cible : ${targetUser.name} (${targetUser.role})`
                  : `Target Profile: ${targetUser.name} (${targetUser.role})`}
              </p>
            </div>
          </div>
        ...
        <div className="px-5 py-2.5 bg-amber-50/80 border-b border-amber-200/70 text-[11px] text-amber-900 flex items-center gap-2 shrink-0">
          ...
          <span>
            {lang === 'FR'
              ? 'Politique de sécurité active : tous les utilisateurs du foyer doivent utiliser une clé FIDO2.'
              : 'Active security policy: All household users must use a FIDO2 security key.'}
          </span>
        </div>
        ...
        <div className="p-5 overflow-y-auto space-y-4">
          ...
          {/* STEP 1: If generated recovery codes to show after enrollment */}
          {generatedCodes && generatedCodes.length > 0 ? (
            <div className="space-y-4 animate-fade-in">
              <div className="p-3.5 rounded-2xl bg-emerald-50 border border-emerald-300 space-y-2">
                ...
                <p className="text-[11px] text-emerald-800 leading-relaxed">
                  {lang === 'FR'
                    ? 'Conservez précieusement ces codes en lieu sûr. Si vous perdez votre clé de sécurité, ils vous permettront de déverrouiller votre profil.'
                    : 'Store these codes in a safe place. If you lose your security key, they allow you to unlock your profile.'}
                </p>
            ...
            <div className="space-y-4 animate-fade-in">
              <div className="p-3.5 rounded-2xl bg-white border border-[#E0D9C8] space-y-2">
                <h4 className="font-bold text-xs text-[#0D3B37]">
                  {lang === 'FR'
                    ? '1. Associer une Clé de Sécurité'
                    : '1. Register a Security Key'}
                </h4>
                <p className="text-[11px] text-[#527470]">
                  {lang === 'FR'
                    ? "Conformément à la politique du foyer, ce profil doit obligatoirement être associé à une clé d'accès (Passkey, Touch ID, YubiKey) pour être activé."
                    : 'In compliance with household policy, this profile must be registered with a Passkey, Touch ID, or YubiKey.'}
                </p>
                ...
                <div className="pt-1">
                  ...
                  <input
                    ...
                    placeholder="e.g. Ma YubiKey 5C NFC"
            ...
            <div className="space-y-4 animate-fade-in">
              {!useRecoveryCode ? (
                <>
                  <div className="p-4 rounded-2xl bg-white border border-[#E0D9C8] text-center space-y-3">
                  ...
                  <div className="text-center">
                    <button
                      ...
                      className="text-[11px] text-teal-800 font-bold hover:underline flex items-center justify-center gap-1 mx-auto cursor-pointer"
                    >
                      <LifeBuoy className="w-3.5 h-3.5" />
                      <span>
                        {lang === 'FR'
                          ? 'Clé indisponible ? Utiliser un code de secours'
                          : 'Lost key? Use emergency recovery code'}
                      </span>
                    </button>
                  </div>
                </>
                ...
                >
                  ...
                  <p className="text-[11px] text-[#527470]">
                    {lang === 'FR'
                      ? 'Entrez l’un de vos codes de secours de 10 caractères générés lors de l’enrôlement FIDO2.'
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      ...
                      className="flex-1 py-2 rounded-xl bg-slate-200 text-slate-800 font-bold text-xs cursor-pointer"
                    >
                      {lang === 'FR' ? 'Retour' : 'Back'}
                    </button>
                    <button
                      ...
                      className="flex-1 py-2 rounded-xl bg-teal-800 hover:bg-teal-900 text-white font-bold text-xs flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                    >
                      {actionLoading ? (
                      ...
                      <span>{lang === 'FR' ? 'Valider' : 'Verify'}</span>
                    </button>
                  </div>
                </form>
              )}
